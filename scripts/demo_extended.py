...
MODEL = os.path.join(ROOT, 'models', 'qkeras_real_model.h5')
...
model = tf.keras.models.load_model(
    MODEL, custom_objects=co)
X_test = np.load(os.path.join(DATA, 'X_test_real.npy'))
y_test = np.load(os.path.join(DATA, 'y_test_real.npy'))
X_all  = np.load(os.path.join(DATA, 'X_real.npy'))
y_all  = np.load(os.path.join(DATA, 'y_real.npy'))
X_train = np.load(os.path.join(DATA, 'X_real.npy'))
...
X = np.load(os.path.join(DATA, 'X_real.npy'))
y = np.load(os.path.join(DATA, 'y_real.npy'))
...
try:
    ...
    print(f"\n  Desktop copy skipped; files are in: {OUT}/")
